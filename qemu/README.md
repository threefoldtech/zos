...
```bash
sudo ./vm.sh -n node-01 -c "farmer_id=47 printk.devmsg=on runmode=dev user=<github username>"
```

where `runmode` is one of `dev` , `test`  or `prod`, 
      `farmer_id` is the id of the farm you registered with `tffarmer`
      `user` is github username provided if a user need to pass ssh-key to the node
