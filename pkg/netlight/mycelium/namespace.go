...
func ensureMyPlumbing(netNS ns.NetNS, myceliumBr string) error {
	if !bridge.Exists(myceliumBr) {
		if _, err := bridge.New(myceliumBr); err != nil {
			return fmt.Errorf("couldn't create bridge %s: %w", myceliumBr, err)
		}
	}

	if err := dumdumHack(myceliumBr); err != nil {
		log.Error().Err(err).Msg("failed to create the dummy hack for mycelium-bridge")
	}

	if !ifaceutil.Exists(MyceliumNSInf, netNS) {
		if _, err := macvlan.Create(MyceliumNSInf, myceliumBr, netNS); err != nil {
			return fmt.Errorf("couldn't create macvlan %s inside: %w", MyceliumNSInf, err)
		}
	}
...
func dumdumHack(myceliumBr string) error {
...
func NewMyNamespace(ns, bridge string) (MyceliumNamespace, error) {
	myNs, err := namespace.GetByName(ns)
	if err != nil {
		return nil, errors.Wrapf(err, "namespace '%s' not found", ns)
	}
	if err := ensureMyPlumbing(myNs, bridge); err != nil {
